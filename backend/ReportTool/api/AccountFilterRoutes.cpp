...
void AccountFilterRoutes::Register(httplib::Server& srv, AppContext* ctx)
{
   ...
   //--- Preview: run the filter against MT5 to show matched-count + samples.
   srv.Post("/api/account-filters/preview", [ctx](const httplib::Request& req, httplib::Response& res){
      ...
      if(!mgr) { SendError(res, 404, "manager not found"); return; }
      ...
      if(body.contains("user_predicate") && !body["user_predicate"].is_null())
      {
         ...
         { SendError(res, 400, std::string("user_predicate: ") + perr); return; }
      ...
      //--- Extra fields to surface in the sample (one per distinct user_predicate field).
      std::vector<std::string> extra_fields;
      if(user_pred) extra_fields = FieldCatalog::CollectPredicateFields(*user_pred);

      json sample_logins = json::array();
      json sample_groups = json::array();
      std::unordered_set<std::string> seen_groups;
      const size_t login_limit = 25;
      ...
      for(size_t i = 0; i < users.size(); ++i)
      {
         ...
         if(seen_groups.size() < group_limit && seen_groups.insert(users[i].group).second)
            sample_groups.push_back(users[i].group);
      }
